...
import requests # this is for downloading files from the internet
import threading #this is for the colab setup
...
# Colab API setup (update with your ngrok URL)
COLAB_URL = "https://d14d-34-48-66-53.ngrok-free.app/process"  # Replace with your Colab server URL
COLAB_LOCK = threading.Lock()  # Prevent race conditions
def swap_face_colab(source_face: Face, target_face_crop: np.ndarray) -> np.ndarray:
    """Offload face-swapping to Colab GPU."""
    _, img_encoded = cv2.imencode('.jpg', target_face_crop)
    files = {
        "source": source_face.embedding.tobytes(),  # Send face embedding
        "target": img_encoded.tobytes()            # Send face crop
    }
    
    try:
        with COLAB_LOCK:
            response = requests.post(COLAB_URL, files=files, timeout=5)
            swapped_face = cv2.imdecode(np.frombuffer(response.content, np.uint8), cv2.IMREAD_COLOR)
        return swapped_face
    except Exception as e:
        print(f"Colab error: {e}")
        return target_face_crop  # Fallback to local CPU
